...
-(void)photoAlertShowAction:(PhotoAction)photoAction IsClip:(BOOL)isClip;{
    ...
    if (!self.photoAlert_photoExtension) {
        ...
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"取消" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction * _Nonnull action) {
            ...
            weakSelf.photoAction_photoExtension(nil);
        }];
...
//        if (!image) {
//            
//            imgUpdate(NO,nil,nil);
//            return;
//        }
//        CommonRequestApi *api = [[CommonRequestApi alloc]init].lxRequestURL(updateUrl)
//        .lxMethod(YTKRequestMethodPOST)
//        .lxUpLoad(YES)
//        .lxRelatedServe(CommonRequestBusinessServe)
//        .lxBody(CompressBigImage(image));
//        
//        [weakSelf.view beginLoading];
//        [api startWithNomalCompletionBlock:^(BOOL issuccess, id responseData) {
//            [weakSelf.view endLoading];
//            imgUpdate(issuccess,image,responseData);
//        }];
//        
//    } IsClip:NO];
}

NSData * CompressBigImage(UIImage *image){
    ...
    while (proportion < 1) {
        ...
        data = UIImageJPEGRepresentation(image, try);
