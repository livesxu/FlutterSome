...
- (UIButton *)sendBtn{
    if (!_sendBtn) {
        ...
        [rightBtn setTitleColor:[UIColor blackColor] forState:UIControlStateDisabled];
...
- (UIButton *)previewBtn
{
    if (!_previewBtn) {
        ...
        [leftBtn setTitleColor:[UIColor blackColor] forState:UIControlStateDisabled];
        leftBtn.enabled = YES;
...
//        [self.view insertSubview:_collectionView = collectionView belowSubview:self.toolBar];//livesxu
...
- (void)viewDidLoad {
    ...
    // 初始化底部ToorBar ---->不使用toorBar,单个图片上传，livesxu
//    [self setupToorBar];
}
...
#pragma mark - 创建右边取消按钮，改cancel为退出，livesxu
- (void)addNavBarCancelButton{
    ...
    [backBtn addTarget:self action:@selector(cancelBtnTouched) forControlEvents:UIControlEventTouchUpInside];
...
//cell被点击会调用,传送LGPhotoAssets *photo一个元素的数组回去,发送图片，livesxu
...
//cell的右上角选择框被点击会调用
- (void) pickerCollectionViewDidSelected:(LGPhotoPickerCollectionView *) pickerCollectionView deleteAsset:(LGPhotoAssets *)deleteAssets indexPath:(NSIndexPath *)indexPath{
